...
def extract_quintet_tree(args):
    ...
    if os.path.exists(dataset_path + '/species_tree_mapped_with_lengths' + str_indices + '.tre'):
        return
    ...
    genes_per_model_condition = glob.glob('data/avian_dataset/*.f200.stripped.tre')

    for gene_tree_path in genes_per_model_condition:
        pattern = "data/avian_dataset/(.*?).f200.stripped.tre"
        output_dir  = re.search(pattern, gene_tree_path).group(1)
        print(output_dir)
